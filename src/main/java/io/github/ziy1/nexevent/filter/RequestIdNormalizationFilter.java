...
public class RequestIdNormalizationFilter implements Filter {
    private static final Set<String> ID_PARAMETERS = Set.of("userId", "eventId", "user_id", "event_id");
        ...
        public NormalizedRequestWrapper(HttpServletRequest request) throws IOException {
            ...
            originalParams.forEach((key, values) -> {
                if (ID_PARAMETERS.contains(key)) {
                    normalizedParams.put(key, Arrays.stream(values)
                            .map(IdNormalizerUtil::normalize)
                            .toArray(String[]::new));
                } else {
